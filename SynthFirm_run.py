...
def log_consist_step_status(step_name, result):
    ...
    if result.cache_hit:
        ...
        print(f"Consist {step_name}: cache miss; executed (run_id={result.run.id})")
...
def main():
    ...
    from utils.Step4_production_and_consumption_forecast import (
    ...
    from utils.Step15_international_B2B_flow_generator import (
        domestic_receiver_assignment,
    )
    ...
    file_path = os.fspath(
        resolve_config_path(config["ENVIRONMENT"]["file_path"], conf_file)
    )
    print(file_path)
    ...
    log_filename = datetime.datetime.now().strftime(
        out_scenario_name + "_run_%Y%m%d.log"
    )
    ...
    run_producer_consumer_generation = config.getboolean(
        "ENVIRONMENT", "enable_producer_consumer_generation"
    )
    ...
    if run_demand_forecast:
        print(
            "including demand forecast in the pipeline and forecast year is "
            + forecast_year
            + "..."
        )

    enable_firm_loc_generation = config.getboolean(
        "ENVIRONMENT", "enable_firm_loc_generation"
    )
    if enable_firm_loc_generation:
        print("including firm location generation in the pipeline...")

    run_supplier_selection = config.getboolean(
        "ENVIRONMENT", "enable_supplier_selection"
    )
    ...
    run_international_flow = config.getboolean(
        "ENVIRONMENT", "enable_international_flow"
    )
    ...
    need_regional_calibration = config.getboolean(
        "ENVIRONMENT", "need_regional_calibration"
    )
    ...
    mesozone_to_faf_file = os.path.join(
        input_path, config["INPUTS"]["mesozone_to_faf_file"]
    )
    c_n6_n6io_sctg_file = os.path.join(
        param_path, config["PARAMETERS"]["c_n6_n6io_sctg_file"]
    )
    employment_per_firm_file = os.path.join(
        param_path, config["PARAMETERS"]["employment_per_firm_file"]
    )
    employment_per_firm_gapfill_file = os.path.join(
        param_path, config["PARAMETERS"]["employment_per_firm_gapfill_file"]
    )
    zip_to_tract_file = os.path.join(
        param_path, config["PARAMETERS"]["zip_to_tract_file"]
    )
    synthetic_firms_no_location_file = os.path.join(
        output_path, config["OUTPUTS"]["synthetic_firms_no_location_file"]
    )
    if assign_enterprises:
        susb_file = os.path.join(param_path, config["PARAMETERS"]["susb_file"])
        county_to_msa_file = os.path.join(
            param_path, config["PARAMETERS"]["county_to_msa_file"]
        )
        costar_file = os.path.join(param_path, config["PARAMETERS"]["costar_file"])
        naics_crosswalk_file = os.path.join(
            param_path, config["PARAMETERS"]["naics_crosswalk_file"]
        )
        firm_enterprise_file = os.path.join(
            output_path, config["OUTPUTS"]["firm_enterprise_file"]
        )

    # inputs/outputs first appear in producer and consumer generation
    BEA_io_2017_file = os.path.join(
        param_path, config["PARAMETERS"]["BEA_io_2017_file"]
    )
    agg_unit_cost_file = os.path.join(
        param_path, config["PARAMETERS"]["agg_unit_cost_file"]
    )
    prod_by_zone_file = os.path.join(
        param_path, config["PARAMETERS"]["prod_by_zone_file"]
    )
    cons_by_zone_file = os.path.join(
        param_path, config["PARAMETERS"]["cons_by_zone_file"]
    )
    ...
    producer_by_sctg_filehead = os.path.join(
        output_path, config["OUTPUTS"]["producer_by_sctg_filehead"]
    )
    consumer_file = os.path.join(output_path, config["OUTPUTS"]["consumer_file"])
    consumer_by_sctg_filehead = os.path.join(
        output_path, config["OUTPUTS"]["consumer_by_sctg_filehead"]
    )
    sample_consumer_file = os.path.join(
        output_path, config["OUTPUTS"]["sample_consumer_file"]
    )
    ...
    print(f"Consist shell command: {build_consist_shell_command(consist_db_path)}")

    # inputs/outputs first appear in demand forecast (optional)
    if forecast_analysis:
        prod_forecast_name = (
            config["PARAMETERS"]["prod_forecast_filehead"] + forecast_year + ".csv"
        )
        prod_forecast_file = os.path.join(param_path, prod_forecast_name)
        cons_forecast_name = (
            config["PARAMETERS"]["cons_forecast_filehead"] + forecast_year + ".csv"
        )
    ...
    synthetic_firms_with_location_file = os.path.join(
        output_path, config["OUTPUTS"]["synthetic_firms_with_location_file"]
    )
    ...
    shipment_by_distance_file = os.path.join(
        param_path, config["PARAMETERS"]["shipment_by_distance_bin_file"]
    )
    shipment_distance_lookup_file = os.path.join(
        param_path, config["PARAMETERS"]["shipment_distance_lookup_file"]
    )
    cost_by_location_file = os.path.join(
        param_path, config["PARAMETERS"]["cost_by_location_file"]
    )
    supplier_selection_param_file = os.path.join(
        param_path, config["PARAMETERS"]["supplier_selection_param_file"]
    )

    # inputs/outputs first appear in shipment size
    cfs_to_faf_file = os.path.join(param_path, config["PARAMETERS"]["cfs_to_faf_file"])
    max_load_per_shipment_file = os.path.join(
        param_path, config["PARAMETERS"]["max_load_per_shipment_file"]
    )

    # inputs/outputs first appear in mode choice
    mode_choice_param_file = os.path.join(
        input_path, config["INPUTS"]["mode_choice_param_file"]
    )
    distance_travel_skim_file = os.path.join(
        param_path, config["PARAMETERS"]["distance_travel_skim_file"]
    )

    # input/outputs first appear in domestic post analysis
    domestic_summary_file = os.path.join(
        output_path, config["OUTPUTS"]["domestic_summary_file"]
    )
    domestic_summary_zone_file = os.path.join(
        output_path, config["OUTPUTS"]["domestic_summary_zone_file"]
    )
    ...
    if run_fleet_generation:
        ...
        # generic inputs
        private_fleet_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["private_fleet_file"]
        )
        for_hire_fleet_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["for_hire_fleet_file"]
        )
        cargo_type_distribution_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["cargo_type_distribution_file"]
        )
        state_fips_lookup_file = os.path.join(
            param_path, config["FLEET_IO"]["state_fips_lookup_file"]
        )

        # scenario-dependent inputs
        private_fuel_mix_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["private_fuel_mix_file"]
        )
        hire_fuel_mix_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["hire_fuel_mix_file"]
        )
        lease_fuel_mix_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["lease_fuel_mix_file"]
        )
        private_stock_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["private_stock_file"]
        )
        hire_stock_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["hire_stock_file"]
        )
        lease_stock_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["lease_stock_file"]
        )
        ev_availability_file = os.path.join(
            param_path, "fleet", config["FLEET_IO"]["ev_availability_file"]
        )

        # fleet outputs
        firms_with_fleet_file = os.path.join(
            output_path,
            fleet_year,
            fleet_scenario_name,
            config["FLEET_IO"]["firms_with_fleet_file"],
        )
        carriers_with_fleet_file = os.path.join(
            output_path,
            fleet_year,
            fleet_scenario_name,
            config["FLEET_IO"]["carriers_with_fleet_file"],
        )
        leasing_with_fleet_file = os.path.join(
            output_path,
            fleet_year,
            fleet_scenario_name,
            config["FLEET_IO"]["leasing_with_fleet_file"],
        )
        firms_with_fleet_mc_adj_files = os.path.join(
            output_path,
            fleet_year,
            fleet_scenario_name,
    ...
    if port_analysis:
        international_summary_file = os.path.join(
            output_path, config["OUTPUTS"]["international_summary_file"]
        )
        international_summary_zone_file = os.path.join(
            output_path, config["OUTPUTS"]["international_summary_zone_file"]
        )

    # those files are only needed for international
    if run_international_flow:
        # input/output appear in international flow
        need_domestic_adjustment = config.getboolean(
            "INPUTS", "need_domestic_adjustment"
        )
        ...
        if forecast_analysis and forecast_year != "2017":
            import_forecast_factor = os.path.join(
                param_path,
                config["PARAMETERS"]["import_forecast_filehead"]
                + forecast_year
                + ".csv",
            )
            export_forecast_factor = os.path.join(
                param_path,
                config["PARAMETERS"]["export_forecast_filehead"]
                + forecast_year
                + ".csv",
            )
        regional_import_file = os.path.join(
            input_path, "port", config["INPUTS"]["regional_import_file"]
        )
        regional_export_file = os.path.join(
            input_path, "port", config["INPUTS"]["regional_export_file"]
        )
        port_level_import_file = os.path.join(
            input_path, "port", config["INPUTS"]["port_level_import_file"]
        )
        port_level_export_file = os.path.join(
            input_path, "port", config["INPUTS"]["port_level_export_file"]
        )
        int_shipment_size_file = os.path.join(
            param_path, config["PARAMETERS"]["int_shipment_size_file"]
        )
        sctg_by_port_file = os.path.join(
            param_path, config["PARAMETERS"]["sctg_by_port_file"]
        )

        import_od = os.path.join(output_path, config["OUTPUTS"]["import_od"])
        export_od = os.path.join(output_path, config["OUTPUTS"]["export_od"])

        int_mode_choice_file = os.path.join(
            input_path, config["INPUTS"]["int_mode_choice_file"]
        )
        import_mode_file = os.path.join(
            output_path, "international", config["OUTPUTS"]["import_mode_file"]
        )
        export_mode_file = os.path.join(
            output_path, "international", config["OUTPUTS"]["export_mode_file"]
        )

        export_with_firm_file = os.path.join(
            output_path, "international", config["OUTPUTS"]["export_with_firm_file"]
        )
        import_with_firm_file = os.path.join(
            output_path, "international", config["OUTPUTS"]["import_with_firm_file"]
        )

    # input/output appear in model validation
    lehd_file = os.path.join(param_path, config["VALIDATION"]["lehd_file"])
    us_county_map_file = os.path.join(
        param_path, config["VALIDATION"]["us_county_map_file"]
    )
    ...
    rail_unit_cost_per_tonmile = float(
        config["MC_CONSTANTS"]["rail_unit_cost_per_tonmile"]
    )
    ...
    truck_unit_cost_per_tonmile_sm = float(
        config["MC_CONSTANTS"]["truck_unit_cost_per_tonmile_sm"]
    )
    mode_choice_spec["truck_unit_cost_sm"] = truck_unit_cost_per_tonmile_sm

    truck_unit_cost_per_tonmile_md = float(
        config["MC_CONSTANTS"]["truck_unit_cost_per_tonmile_md"]
    )
    mode_choice_spec["truck_unit_cost_md"] = truck_unit_cost_per_tonmile_md

    truck_unit_cost_per_tonmile_lg = float(
        config["MC_CONSTANTS"]["truck_unit_cost_per_tonmile_lg"]
    )
    ...
    synthfirm_scenario_config = {
        "scenario_name": scenario_name,
        "out_scenario_name": out_scenario_name,
        "synthfirm_config": synthfirm_config,
        "enabled_steps": {
            "firm_generation": run_firm_generation,
            "producer_consumer_generation": run_producer_consumer_generation,
            "demand_forecast": run_demand_forecast,
            "firm_location_generation": enable_firm_loc_generation,
            "supplier_selection": run_supplier_selection,
            "shipment_size_generation": run_size_generation,
            "mode_choice": run_mode_choice,
            "post_analysis": run_post_analysis,
            "fleet_generation": run_fleet_generation,
            "international_flow": run_international_flow,
            "model_validation": run_model_validation,
        },
    }
    synthfirm_scenario_name = (
        f"synthfirm_{out_scenario_name}_"
    ...
    with tracker.scenario(
        ...
        if run_firm_generation:
            step1_spec = build_step1_consist_spec(
                ...
                synthetic_enterprise_file=firm_enterprise_file
                if assign_enterprises
                else None,
                cbp_file=cbp_file,
            ...
            step1_archive = archive_consist_run_outputs(
                tracker,
                step1_result.run.id,
                consist_recovery_root,
                output_keys=step1_spec["output_paths"],
            )
            baseline_synthetic_firms = step1_archive.outputs["synthetic_firms"]
        ...
        if run_producer_consumer_generation:
            ...
            step2_archive = archive_consist_run_outputs(
                tracker,
                step2_result.run.id,
                consist_recovery_root,
                output_keys=step2_spec["output_paths"],
            )
            synthetic_wholesaler = step2_archive.outputs["wholesaler"]
            synthetic_producer = step2_archive.outputs["producer"]
            io_filtered = step2_archive.outputs["io_filtered"]
            wholesale_cost_factor = step2_archive.outputs["wholesale_cost_factor"]
            ...
            step3_archive = archive_consist_run_outputs(
                tracker,
                step3_result.run.id,
                consist_recovery_root,
                output_keys=step3_spec["output_paths"],
            )
            synthetic_consumer = step3_archive.outputs["consumer"]
        ...
        if run_supplier_selection:
            supplier_selection(
                mesozone_to_faf_file,
                shipment_by_distance_file,
                shipment_distance_lookup_file,
                cost_by_location_file,
                producer_file,
                consumer_file,
                cfs_to_faf_file,
                max_load_per_shipment_file,
                sctg_group_file,
                supplier_selection_param_file,
                output_path,
            )

        ##### Step 7 - shipment size generation
        if run_size_generation:
            shipment_size_generation(
                mesozone_to_faf_file,
                max_load_per_shipment_file,
                region_code,
                output_path,
        ...
        if run_mode_choice:
            mode_choice_model(
                mode_choice_param_file,
                mesozone_to_faf_file,
                distance_travel_skim_file,
                mode_choice_spec,
                output_path,
                number_of_processes,
            )
        ...
        if run_fleet_generation:
            if need_regional_calibration:
                ...
                firm_fleet_generator(
                    ...
                    private_fleet_file,
                    for_hire_fleet_file,
                    cargo_type_distribution_file,
                    state_fips_lookup_file,
                    private_fuel_mix_file,
                    hire_fuel_mix_file,
                    lease_fuel_mix_file,
                    private_stock_file,
                    hire_stock_file,
                    lease_stock_file,
                    firms_with_fleet_file,
                    carriers_with_fleet_file,
                    leasing_with_fleet_file,
                    ev_availability_file,
                    output_path,
                    need_regional_calibration,
                    regional_variable,
                )

                firm_fleet_generator_post_mc(
                    int(fleet_year),
                    fleet_name,
                    regulations,
                    synthetic_firms_with_location_file,
        ...
        if run_international_flow:
            ...
            if need_domestic_adjustment:
                print(
                    "Use international flow generation with destination adjustment..."
                )
                if forecast_analysis and forecast_year != "2017":
                    international_demand_generation(
                        c_n6_n6io_sctg_file,
                        sctg_by_port_file,
                        sctg_group_file,
                        int_shipment_size_file,
                        regional_import_file,
                        regional_export_file,
                        port_level_import_file,
                        port_level_export_file,
                        need_domestic_adjustment,
                        import_od,
                        export_od,
                        output_path,
                        forecast_year,
                ...
                print(
                    "Use international flow generation without destination adjustment..."
                )
                if forecast_analysis and forecast_year != "2017":
                    international_demand_generation(
                        c_n6_n6io_sctg_file,
            ...
            if regional_analysis:
                international_mode_choice(
                    int_mode_choice_file,
                    distance_travel_skim_file,
                    import_od,
                    export_od,
                    import_mode_file,
