...
from utils.pre_processor_boston import boston_employment_calibration # BOSTON pre-run calibration
...
from utils.post_processor_boston import boston_bus_location_calibration # BOSTON post-run calibration
...
def main():
    ...
    parser.add_argument("--config", type = str, help = "config file name", 
                        default= 'configs/Boston_base.conf')
    ...
    # load module to run
    run_boston_pre_process = config.getboolean('ENVIRONMENT', 'enable_boston_pre_calibration')
    if run_boston_pre_process:
        print('including pre-run employment calibration in the pipeline for Boston...')
    ...
    run_boston_post_process = config.getboolean('ENVIRONMENT', 'enable_boston_post_run_calibration')
    if run_boston_post_process:
        print('including post-run business location calibration in the pipeline for Boston...')
    ...
    if need_regional_calibration:
        regional_variable_str = config['CALIBRATION']['regional_variable']
        regional_variable = [var for var in regional_variable_str.split(',')]

        # load region-specific data for calibration
        uncalibrated_mzemp_file = os.path.join(input_path, config['CALIBRATION']['uncalibrated_mzemp_file'])
        taz_file = os.path.join(input_path, config['CALIBRATION']['tazs'])
#         psrc_parcel_file = os.path.join(input_path, config['CALIBRATION']['psrc_parcel_file'])
...
#         cleaned_parcel_file = os.path.join(input_path, config['CALIBRATION']['cleaned_parcel_file'])
    ...
    print('----------------------------------------------')
    ######## HERE START THE SYNTHFIRM RUN!!####

    ##### Step 0 - Pre-model employment calibration
    ...
    ##### Step 5.1 -  calibrate synthetic firm location within BOSTON region
    if run_boston_post_process:
        boston_bus_location_calibration(taz_file, mzemp_file,
                                          synthetic_firms_with_location_file,
                                          plot_path)
