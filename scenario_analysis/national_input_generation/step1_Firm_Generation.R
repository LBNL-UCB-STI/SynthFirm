...
cbp <- data.table::fread("./inputs_national/data_emp_cbp_imputed.csv", h = T)
# mzemp <-
#   data.table::fread("./inputs/data_mesozone_emprankings.csv", h = T)
employment_per_firm <- 
  data.table::fread("./inputs_national/employment_by_firm_size_naics.csv", h = T)
employment_per_firm_gapfill <- 
  data.table::fread("./inputs_national/employment_by_firm_size_gapfill.csv", h = T)
# for_prod <-
...
write.csv(firms, './outputs_national/synthetic_firms.csv', row.names=FALSE)
