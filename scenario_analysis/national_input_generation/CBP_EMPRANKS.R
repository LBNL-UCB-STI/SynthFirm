...
naics_file_name = paste0(selected_region, '_naics.csv') # employment data
f2 = data.table::fread(naics_file_name,colClasses = list(character=c("GEOID","metalayer_id")),h=T)
f2 = f2 %>% select(-c(metalayer_id)) %>% as_tibble()
...
naics_df6 = na.omit(naics_df4)
naics_df6$MESOZONE = naics_df6$GEOID
naics_df6$CBPZONE1 = naics_df6$GEOID
