...
out_file_name = paste0(region_name, "_county.geojson")
...
faf_lookup <- faf_lookup %>% select(FAFID, CBPZONE1) %>% as_tibble()
faf_lookup_unique <- faf_lookup %>% distinct()
faf_lookup_unique <- faf_lookup_unique %>% mutate(MESOZONE = CBPZONE1)
