...
import_mode_file = os.path.join(int_out_dir, 'import_OD_with_mode.csv')
export_mode_file = os.path.join(int_out_dir, 'export_OD_with_mode.csv')

producer_file = os.path.join(file_path, output_dir, 'synthetic_producers.csv')
consumer_file = os.path.join(file_path, output_dir, 'synthetic_consumers.csv')
mesozone_to_faf_file = os.path.join(file_path, input_dir, "zonal_id_lookup_final.csv") # zonal ID lookup table 
sctg_group_file = os.path.join(file_path, parameter_dir, "SCTG_Groups_revised_V2.csv" )# Commodity type to group lookup (pre-defined)
export_with_firm_file = os.path.join(int_out_dir, 'export_OD_with_seller_' + str(fleet_year) + '.csv')
import_with_firm_file = os.path.join(int_out_dir, 'import_OD_with_buyer_' + str(fleet_year) + '.csv')


hire_fuel_mix_file = os.path.join(file_path, parameter_dir, 'fleet/hire_fuel_mix_scenario.csv')
ev_availability_file = os.path.join(file_path, parameter_dir, 'fleet/synthfirm_ev_availability.csv')
...
import_output_with_mode = read_csv(import_mode_file)
export_output_with_mode = read_csv(export_mode_file)
