...
def validate_commodity_flow(sctg_group_file,
                            faf_data_file, cfs_data_file,
                            domestic_summary_file,
                            analysis_year, plot_dir, 
                            region_code=None, focus_region=None,                     
                            port_analysis = False, 
                            international_summary_file = None):
    print(f'Starting commodity flow validation for year {analysis_year}...')
    ...
    if focus_region is not None:
        in_zone = ((faf_data_domestic['Origin'].isin(focus_region)) | \
                     (faf_data_domestic['Destination'].isin(focus_region)))
        faf_load_in_zone = faf_data_domestic.loc[in_zone]
        faf_zone_total = faf_load_in_zone.loc[:, 'Load'].sum() * us_ton_to_ton
        print(f'Total load from FAF zone {focus_region}')
        print(faf_zone_total)
        
        model_in_zone = ((modeled_data['Origin'].isin(focus_region)) | \
                     (modeled_data['Destination'].isin(focus_region)))
        modeled_data_in_zone = modeled_data[model_in_zone]
        model_zone_total = modeled_data_in_zone.loc[:, 'Load'].sum() * us_ton_to_ton
        print(f'Total load from SynthFirm zone {focus_region}')
        ...
        print(f'Total truck load from FAF zone {focus_region}')
        ...
        print(f'Total truck load from SynthFirm zone {focus_region}')
