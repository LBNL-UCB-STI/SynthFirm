...
g1_prods = data.table::fread("./outputs/prods_sctg4.csv", h = T)
g1_consm = data.table::fread("./outputs/consumers_sctg4.csv", h = T)
...
for (i in 1:length(g1_split)) {
  ...
  saveRDS(sctg1_od, paste0("./outputs/sctg4_od", i, ".rds"))
  rm(sctg1_od)
  rm(test0)
  gc()
}
...
for (i in 1:chunk_size) {
  ...
  od_list[[i]] = readRDS(paste0("./outputs/sctg4_od", i, ".rds"))
}

od_df = od_list %>% bind_rows()
saveRDS(od_df, paste0("./outputs/sctg4_od.rds"))
sapply(paste0("./outputs/sctg4_od", 1:chunk_size, ".rds"), unlink)
