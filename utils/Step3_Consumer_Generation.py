...
consumer_value_fraction_by_location = \
    read_csv(os.path.join(file_path, parameter_dir, cons_by_zone_file))
...
consumers_out = pd.merge(consumers_out,
                         ...
                         how = 'left')
# <codecell>
synthetic_consumer_by_sctg_filehead = "consumers_sctg" 
...
for i in range(5):
  ...
  g1_cons.to_csv(os.path.join(file_path, output_dir, synthetic_consumer_by_sctg_filehead + str(i+1) + ".csv"), index = False)
