...
private_carrier_size_distribution <- data.table::fread("./inputs/private_fleet_size_distribution.csv", h = T)
for_hire_carrier_size_distribution <- data.table::fread("./inputs/for_hire_fleet_size_distribution.csv", h = T)
for_lease_carrier_size_distribution <- data.table::fread("./inputs/for_lease_fleet_size_distribution.csv", h = T)
...
write.csv(carrier_industry, './outputs/synthetic_for_hire_carriers.csv', row.names=FALSE)
...
write.csv(lease_industry, './outputs/synthetic_veh_lease_firms.csv', row.names=FALSE)

firms_out <- firms %>% filter(!Industry_NAICS6_Make %in% c('492000', '484000', '532100')) %>% as_tibble()
write.csv(firms_out, './outputs/synthetic_firms_with_fleet.csv', row.names=FALSE)
