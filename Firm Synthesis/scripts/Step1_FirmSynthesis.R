...
cbp[CBPZONE <= 130, MESOZONE := CBPZONE + 20000L]
...
setnames(
  ...
  c("Industry_NAICS6_Make", "Industry_NAICS6_Use", "ProVal")
)
...
for (i in 1:nrow(whlnaicscombs)) {
  ...
  wholesalers[NAICS == whlnaicscombs$NAICS[i] &
...
io <-
  io[Industry_NAICS6_Make %in% unique(producers$NAICS)] #focus on just producers of transported commodities
...
io <-
  io[CumPctProVal > 1 - model$scenvars$provalthreshold,] #select suppliers including the first above the threshold value
...
for_cons[PurchaseAmountTons > 500000000, est := ceiling(PurchaseAmountTons /
...
firms_sum[["firmsempbysctg"]] <-
  merge(sctgcat, cbp[, list(Establishments = .N, Employment = sum(Emp)), by =
...
consumers_summary <-
  merge(sctgcat, consumers[, list(Consumers = .N,
                                  InputRequirements = sum(PurchaseAmountTons)), by = Commodity_SCTG], "Commodity_SCTG", all.y =
          T)
