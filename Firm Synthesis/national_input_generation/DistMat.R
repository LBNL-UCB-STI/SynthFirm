...
set.seed(0)
path2file <-
  "/Users/xiaodanxu/Documents/SynthFirm.nosync/national_GIS"
setwd(path2file)

f1 = sf::st_read("national_freight_centroids.geojson")
county_faf_lookup <- data.table::fread("national_FAFCNTY.csv", h = T)
county_faf_lookup <- county_faf_lookup %>% select(FAFID, CBPZONE) %>% as_tibble()
require(sp)
...
centroid_distance_long <- merge(centroid_distance_long, county_faf_lookup, by.x = 'OriginZone', by.y = 'CBPZONE', all.x = TRUE)
setnames(centroid_distance_long, 'FAFID', 'orig_FAFID')

centroid_distance_long <- merge(centroid_distance_long, county_faf_lookup, by.x = 'dest_zone', by.y = 'CBPZONE', all.x = TRUE)
setnames(centroid_distance_long, 'FAFID', 'dest_FAFID')
...
# f2 = f2 %>% filter(DistMiles > 0)
data.table::fwrite(centroid_distance_long, "national_od_dist.csv")
