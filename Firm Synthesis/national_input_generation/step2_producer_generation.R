...
firms <- data.table::fread("./outputs_national/synthetic_firms.csv", h = T) # 8,390,414 FIRMS
for_prod <-
...
io <- io[io$ProVal>0,]
write.csv(io, './outputs_national/io_summary.csv', row.names=FALSE)
...
for_prod[, MESOZONE := CBPZONE + 100000L] # not conflicting with any county fips code
...
write.csv(wholesalers_with_value, './outputs_national/synthetic_wholesaler.csv', row.names=FALSE)
write.csv(producers, './outputs_national/synthetic_producers.csv', row.names=FALSE)
write.csv(io_no_wholesale, './outputs_national/data_2017io_filtered.csv', row.names=FALSE)
