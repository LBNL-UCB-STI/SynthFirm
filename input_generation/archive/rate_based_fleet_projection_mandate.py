...
for year in list_of_years:
    ...
    # fleet_mix_next_year.loc[:, 'scrap_weight'] = \
    #     fleet_mix_next_year.groupby(['sourceTypeID'])['scrap_weight'].transform('sum')/ \
    #         fleet_mix_next_year.groupby(['sourceTypeID'])['ScrappedVeh'].transform('mean')
    # fleet_mix_next_year
            
    # fleet_mix_next_year.loc[:, 'veh_to_scrap'] = \
    #     fleet_mix_next_year.loc[:, 'scrap_weight'] * \
    #         fleet_mix_next_year.loc[:, 'ScrappedVeh']
    # scrappage_goal = fleet_total_by_year[['sourceTypeID', 'ScrappedVeh']]
    ...
    # break
fleet_mix_by_year.to_csv(os.path.join(path_to_moves, 'turnover', 'age_distribution_static_mandate.csv'),
                          index = False)
...
plt.savefig(os.path.join(path_to_moves, 'plot_forecast', 'com_age_distribution_mandate.png'), dpi = 300,
            bbox_inches = 'tight')
plt.show()
