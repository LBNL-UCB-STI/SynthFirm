...
mode_choice_param_file = 'freight_mode_choice_5alt_austin.csv'
...
for sctg in c.list_of_sctg_group:
    ...
    filelist = [file for file in os.listdir(c.input_dir) if file.startswith(file_start)]
    all_modeled_OD_by_sctg = pd.concat([read_csv(c.input_dir + f, low_memory=False) for f in filelist ])
